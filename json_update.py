...
def process_row(row, cursor, ftp_host, ftp_username, ftp_password):
    id, app_name, web_url = row[0], row[1], f'http://web2app.appcollection.in/V07/redirect.php?id={row[0]}'
    print(id, app_name, web_url)

    # Generate configuration content for the app
    content = create_app_configuration(app_name, web_url)

    # Upload to FTP
    filename = "mightyweb.json"
    upload_to_ftp(ftp_host, ftp_username, ftp_password, filename, content, id)

def create_app_configuration(app_name, web_url):
    config = {
      "appconfiguration": {
          "app_name": app_name,
          "url": web_url,
