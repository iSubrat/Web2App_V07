...
def process_row(row, cursor, ftp_host, ftp_username, ftp_password):
    id, app_name, web_url, app_logo = row[0], row[1], f'http://web2app.appcollection.in/V07/redirect.php?id={row[0]}', f'https://appcollection.in/InstantWeb2App/V07/uploads/{row[6]}'
    print(id, app_name, web_url)

    # Generate configuration content for the app
    content = create_app_configuration(app_name, web_url, app_logo)

    # Upload to FTP
    filename = "mightyweb.json"
    upload_to_ftp(ftp_host, ftp_username, ftp_password, filename, content, id)

def create_app_configuration(app_name, web_url, app_logo):
    config = json.dumps({
                "appconfiguration": {
                    "app_name": app_name,
                    "url": web_url,
